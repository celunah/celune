...
TEXT_CONFIG = {
    ...
    "assessment": {
        "warning_short_audio": "This report may not be comprehensive enough to determine how Celune is actually performing. Please provide a longer clip.",
        ...
        "pitch_desc_mid_female": "may be higher than expected. If measuring Celune's upbeat tone, this is safe to ignore",
...
def _join_trait_names(trait_names: list[str]) -> str:
    ...
    if len(display_names) == 2:
        return f"{display_names[0]} and {display_names[1]}".lower()
    return ", ".join(display_names[:-1]).lower() + f", and {display_names[-1]}".lower()
...
def _summarize_trait_status(traits: dict) -> tuple[str, str]:
    ...
    if high_traits:
        joined = _join_trait_names(high_traits)
        template_key = (
    ...
    if warning_traits:
        joined = _join_trait_names(warning_traits)
        template_key = (
...
def write_report(
    ...
    with output_path.open("w", encoding="utf-8") as file_handle:
        ...
        for trait, score in traits.items():
            ...
            file_handle.write(f"  {_trait_label(trait):<16} : {score:.4f}  [{bar}]\n")
        file_handle.write("\n")
