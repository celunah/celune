...
def main() -> int:
    ...
    def _worker_fatal() -> None:
        ...
        send_message(
            sys.stdout.buffer, cast(WorkerMessage, {"ok": True, "fatal": True})
        )
    ...
    while True:
        ...
        try:
            ...
            response = cast(WorkerResponse, {"ok": False, "error": str(error)})
        send_message(sys.stdout.buffer, cast(WorkerMessage, response))
